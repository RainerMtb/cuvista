...
static const X86Info cpu = GetX86Info();
 ...
 //CPU Device Info
std::string DeviceInfo<CpuFrame>::getName() const {
...
std::string DeviceInfo<CpuFrame>::getNameShort() const {
	return "Cpu";
}

//full name of this cpu
std::string DeviceInfo<CpuFrame>::getCpuName() const {
	return cpu.brand_string;
}

std::shared_ptr<MovieFrame> DeviceInfo<CpuFrame>::createClass(MainData& data, MovieReader& reader, MovieWriter& writer) {
	return std::make_shared<CpuFrame>(data, reader, writer);
}

//CPU Device Info
std::string DeviceInfo<AvxFrame>::getName() const {
	X86Microarchitecture arch = GetX86Microarchitecture(&cpu);
	return std::format("AVX 512, {}", cpu.brand_string);
}

std::string DeviceInfo<AvxFrame>::getNameShort() const {
	return "Avx 512";
}

//check if avx512 is available
bool DeviceInfo<AvxFrame>::hasAvx512() const {
	return cpu.features.avx512f & cpu.features.avx512vl & cpu.features.avx512bw;
}

std::shared_ptr<MovieFrame> DeviceInfo<AvxFrame>::createClass(MainData& data, MovieReader& reader, MovieWriter& writer) {
	return std::make_shared<AvxFrame>(data, reader, writer);
}


//OPENCL Info
std::string DeviceInfo<OpenClFrame>::getName() const {
	std::string name = device.getInfo<CL_DEVICE_NAME>();
	std::string vendor = device.getInfo<CL_DEVICE_VENDOR>();
	return std::format("OpenCL, {}, {}", name, vendor);
}

std::string DeviceInfo<OpenClFrame>::getNameShort() const {
	return "OpenCL";
}

std::shared_ptr<MovieFrame> DeviceInfo<OpenClFrame>::createClass(MainData& data, MovieReader& reader, MovieWriter& writer) {
	return std::make_shared<OpenClFrame>(data, reader, writer);
}

std::ostream& operator << (std::ostream& os, const DeviceInfo<OpenClFrame>& info) {
...
//CUDA Info
std::ostream& operator << (std::ostream& os, const DeviceInfo<CudaFrame>& info) {
...
std::string DeviceInfo<CudaFrame>::getName() const {
	return std::format("Cuda, {}, Compute {}.{}", props.name, props.major, props.minor);
}

std::string DeviceInfo<CudaFrame>::getNameShort() const {
	return "Cuda";
}

std::shared_ptr<MovieFrame> DeviceInfo<CudaFrame>::createClass(MainData& data, MovieReader& reader, MovieWriter& writer) {
