...
void FrameResult::computeTransform(std::vector<PointResult>& results, ThreadPool& threadPool, int64_t frameIndex, RNGbase* rng) {
	compute(results, threadPool, frameIndex, rng);
}
...
void FrameResult::computeExperimental(std::vector<PointResult>& results, ThreadPool& threadPool, int64_t frameIndex, RNGbase* rng) {
	...
	auto sortByDist = [] (const PointContext& p1, const PointContext& p2) { return p1.delta < p2.delta; };
	auto stretchFun = [] (double x) { return 5.0 - 2.0 / std::sqrt(0.25 + 0.125 * x); };   //length factor of ellipse

	//util::ConsoleTimer timer("transform");
	mAffineSolver->reset();
	mAffineSolver->frameIndex = frameIndex;

	//only valid points
	mFiniteEnd = std::copy_if(results.cbegin(), results.cend(), mFiniteResults.begin(), [] (const PointResult& res) { return res.isValid(); });
	mFiniteCount = std::distance(mFiniteResults.begin(), mFiniteEnd);
	mConsList.clear();
	//std::cout << "frame " << frameIndex << ", finite " << siz << std::endl;

	if (mFiniteCount > cMinConsensPoints) {
		//calculate vector parameters once
		double averageLength = 0.0;
		for (auto it = mFiniteResults.begin(); it != mFiniteEnd; it++) {
			it->length = std::sqrt(sqr(it->u) + sqr(it->v));
			it->heading = std::atan2(it->v, it->u) * 180.0 / std::numbers::pi;
			it->stretch = stretchFun(it->length);
			it->isConsens = false;
			averageLength += it->length;
		}

		averageLength /= mFiniteCount;
		for (auto it = mFiniteResults.begin(); it != mFiniteEnd; it++) {
			//create PointContext and set delta value to deviation from average length
			PointContext pc = *it;
			pc.delta = std::abs(it->length - averageLength);
			mConsList.push_back(pc);
		}
		//discard 10% outsiders compared to average length
		std::sort(mConsList.begin(), mConsList.end(), sortByDist);
		mConsList.resize(mFiniteCount * cCutoffPercent / 100);

		//try to find consens with smallest error ellipse, increasing area when no appropriate consens can be found
		bool continueLoop = true;
		ptrdiff_t consCount = 0;
		for (size_t k = 0; k < cConsensRadius.size() && continueLoop; k++) {
			...
			for (int i = 0; i < cConsLoopCount && consCount < siz; i++) {
				auto iterEnd = mConsList.begin() + siz;

				//similar transform based on current set
				mBestTransform = mAffineSolver->computeSimilar(mConsList.begin(), iterEnd);

				//check consensus set
				computePointContext(mConsList.begin(), iterEnd, mBestTransform, radius);
				std::sort(mConsList.begin(), iterEnd, sortByDist);
				auto fcn = [] (const PointContext& p) { return p.delta < p.distanceEllipse; };
				consCount = std::count_if(mConsList.begin(), iterEnd, fcn);
				siz = siz * cCutoffPercent / 100;
				//std::cout << "frame " << frameIndex << ", loop " << k << "-" << i << ", numCons " << numCons << std::endl;
			}
			continueLoop = 100.0 * consCount / mFiniteCount < cRetryPercentage;
		}

		if (!continueLoop) {
			mConsList.resize(consCount);
			...
			std::vector<PointResult> samples(2);
			...
			for (int k = 0; k < cLoopCount; k++) {
				...
				std::sample(mFiniteResults.begin(), mFiniteEnd, samples.begin(), samples.size(), *rng);
				const AffineTransform& trf = mAffineSolver->computeSimilarDirect(samples[0], samples[1]);

				//build consensus set
				std::vector<PointContext> cons;
				computePointContext(mConsList.begin(), mConsList.end(), trf, radius);
				...
				if (cons.size() > cMinCons) {
					AffineTransform trf2 = mAffineSolver->computeSimilar(cons.begin(), cons.end());
					computePointContext(mConsList.begin(), mConsList.end(), trf2, radius);
					cons.clear();
					std::copy_if(mConsList.begin(), mConsList.end(), std::back_inserter(cons), fcn);
...
void FrameResult::computePointContext(IT begin, IT end, const AffineTransform& trf, double radius) {
	for (auto it = begin; it != end; it++) {
		const PointResult& pr = *it->ptr;
		auto tp = trf.transform(pr.x, pr.y);            //apply transform to point
		it->tx = tp.first; it->ty = tp.second;          //store transformed point

		double px = pr.x + pr.u, py = pr.y + pr.v;      //actual computed point
		double dx = it->tx - px, dy = it->ty - py;      //error vector
		it->delta = std::sqrt(sqr(dx) + sqr(dy));       //error distance
		it->deltaAngleCos = (pr.u * dx + pr.v * dy) / (it->delta * pr.length); //cos(angle) = (a*b)/(|a|*|b|)
		it->deltaAngle = std::acos(it->deltaAngleCos) * 180.0 / std::numbers::pi;

		//distance to ellipse
		//ellipse in polar coordinates d^2 = b^2/(1-e^2*cos(theta)^2)
		double epsilon2 = 1.0 - 1.0 / sqr(pr.stretch);
		it->distanceEllipse = std::sqrt(sqr(radius) / (1.0 - epsilon2 * sqr(it->deltaAngleCos)));
	}
}
...
void FrameResult::compute(std::vector<PointResult>& results, ThreadPool& threadPool, int64_t frameIndex, RNGbase* rng) {
	...
	if (mFiniteCount > cMinConsensPoints) {
		...
		mConsList.resize(mFiniteCount * 90 / 100);
		...
		for (int i = 0; i < cConsLoopCount; i++) {
			...
			mBestTransform = mAffineSolver->computeSimilar(mConsList.begin(), mConsList.end());
